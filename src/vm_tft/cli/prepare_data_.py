...
def main() -> None:
    ...
    # processed files
    all_out  = PROC_DIR / "vm_tft_all_new.csv"
    test_out = PROC_DIR / "vm_tft_test_new.csv"
    ...
    print(f"Remaining wells: {merged_train['well_id'].nunique()}")
    print(f"Months per well mean: {merged_train.groupby('well_id')['date'].count().mean()}")
    print(f"Months per well min: {merged_train.groupby('well_id')['date'].count().min()}")
    print(f"Months per well max: {merged_train.groupby('well_id')['date'].count().max()}")
    ...
    bad_wells = [154212,154777,154790,154929,154931,
                ...
                157331,157567, 157862,157871,157872,157973,
    ...
    df_train_reindexed_train = df_train_reindexed[n_per_well >= need_len].copy()
    ...
    bad_wells = [157567,157733,160428,160429,160691,160943,162172,162387,162388,162486,162638]

    merged_test = merged_test.loc[~merged_test.well_id.isin(bad_wells)]
    
    print(f"Remaining wells: {merged_test['well_id'].nunique()}")
    print(f"Months per well mean: {merged_test.groupby('well_id')['date'].count().mean()}")
